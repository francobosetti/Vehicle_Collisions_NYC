...
<<echo=FALSE, message=FALSE, warning=FALSE>>=
read.csv(file = "data/Motor_Vehicle_Collisions_-_Crashes.csv") %>%
mutate(CRASH.DATE=as.Date(CRASH.DATE, format = "%m/%d/%Y"),
       ...
       HOUR=as.numeric(sub("\\:.*", "", CRASH.TIME))) -> nyc
...
# Combine 
#twitter <- rbind(twitter_train, twitter_test) old method, no deplyr
...
longs <- strsplit(sp_limits$the_geom, " ") %>%
  ...
  as.numeric()
loc<- paste("(", round(lats, digits = 4), ", ", round(longs, digits = 4), ")", sep = "")
sp_lim_proc <- mutate(sp_limits, latitude = lats, longitude = longs, ROUND.LOCATION =loc) %>%
...
combined <- # add speed limit data to nyc dataset
  ...
  inner_join(dplyr::select(sp_limits_filt, SPEED.LIMIT, ROUND.LOCATION), by="ROUND.LOCATION")
@
...
<<echo=FALSE, message=FALSE, warning=FALSE>>=
nyv <- read.csv(file = "data/Motor_Vehicle_Collisions_-_Vehicles_20250930.csv")
...
text(bp, -max(fivemost)*0.05, names(fivemost), srt = 45, xpd = TRUE, adj = 1, cex = 0.8)
...
<<echo=FALSE, message=FALSE, warning=FALSE, fig.height=15, fig.width=11>>=
# First reshape the data to long format for stacking
# First reshape the data to long format for stacking
library(tidyr)
nyc_long <- nyc %>%
  dplyr::select(YEAR, BOROUGH, 
         Pedestrian_Injured = NUMBER.OF.PEDESTRIANS.INJURED,
         Pedestrian_Killed = NUMBER.OF.PEDESTRIANS.KILLED,
         Cyclist_Injured = NUMBER.OF.CYCLIST.INJURED,
         Cyclist_Killed = NUMBER.OF.CYCLIST.KILLED,
         Motorist_Injured = NUMBER.OF.MOTORIST.INJURED,
         Motorist_Killed = NUMBER.OF.MOTORIST.KILLED) %>%
  pivot_longer(cols = c(Pedestrian_Injured, Pedestrian_Killed,
                       Cyclist_Injured, Cyclist_Killed,
                       Motorist_Injured, Motorist_Killed),
               names_to = "injury_type", 
               values_to = "count")

nyc_summarized <- nyc_long %>%
  filter(BOROUGH != "") %>%
  group_by(YEAR, BOROUGH, injury_type) %>%
  summarise(total_count = sum(count, na.rm = TRUE)) %>%
  ungroup()


injuries_summarized <- nyc_long %>%
  filter(BOROUGH != "", grepl("_Injured", injury_type)) %>%
  group_by(YEAR, BOROUGH, injury_type) %>%
  summarise(total_count = sum(count, na.rm = TRUE)) %>%
  ungroup()

ggplot(injuries_summarized) +
  aes(x = BOROUGH, y = total_count, fill = injury_type) +
  geom_col() +
  facet_wrap(~ YEAR, ncol = 4) +
  scale_fill_brewer(palette = "BuGn") +
  labs(
    title = "Injuries by Borough, Road User, and Year",
    x = "Borough",
...
<<echo=FALSE, message=FALSE, warning=FALSE, fig.height=15, fig.width=11>>=
fatalities_summarized <- nyc_long %>%
  filter(BOROUGH != "", grepl("_Killed", injury_type)) %>%
  group_by(YEAR, BOROUGH, injury_type) %>%
  summarise(total_count = sum(count, na.rm = TRUE)) %>%
  ungroup()

ggplot(fatalities_summarized) +
  aes(x = BOROUGH, y = total_count, fill = injury_type) +
  geom_col() +
  facet_wrap(~ YEAR, ncol = 4) +
  scale_fill_brewer(palette = "Set2") +
  labs(
    title = "Fatalities by Borough, Road User, and Year",
    x = "Borough",
...
<<echo=FALSE, message=FALSE, warning=FALSE>>=
nyc$injury_severity <- ifelse(nyc$NUMBER.OF.PERSONS.KILLED > 0, 
                              "FATAL/DIED",
...
<<echo=FALSE, message=FALSE, warning=FALSE>>=
prop_df <- as.data.frame(prop_table1)
names(prop_df) <- c("Borough", "Severity", "Proportion")

ggplot(data = prop_df) + 
  aes(x = Borough, y = Proportion, fill = Severity) + 
  ...
  geom_text(aes(label = paste0(round(Proportion * 100, 1), "%")), 
            ...
            color = "gray30") +
  labs( 
    title = "Crash Severity Distribution by Borough", 
    subtitle = "Proportion of crashes by injury outcome", 
    x = "Borough", 
    y = "Proportion", 
    fill = "Severity" 
  ) + 
  scale_fill_manual(values = c("FATAL/DIED" = "#2D3748",   
                                "INJURY" = "#718096",        
                                "NO INJURY" = "#CBD5E0")) +  
  theme_minimal() + 
  theme( 
...
crash_summary <- injury_data %>%
  mutate(
    crash_type = case_when(
      NUMBER.OF.PERSONS.KILLED > 0 & NUMBER.OF.PERSONS.INJURED > 0 ~ "Fatal & Injury",
      NUMBER.OF.PERSONS.KILLED > 0 & NUMBER.OF.PERSONS.INJURED == 0 ~ "Fatal Only",
      NUMBER.OF.PERSONS.KILLED == 0 & NUMBER.OF.PERSONS.INJURED > 0 ~ "Injury Only",
      TRUE ~ "No Casualties"
    )
  ) %>%
  count(crash_type) %>%
...
ggplot(data = crash_summary) +
  aes(x = n, y = reorder(crash_type, n), fill = crash_type) +
  geom_bar(stat = "identity") +
  geom_text(aes(label = paste0(format(n, big.mark = ","), " (", percentage, "%)")),
            hjust = -0.1, size = 4, fontface = "bold") +
  labs(
    title = "Distribution of NYC Crashes by Casualty Type",
    y = "",
    x = "Number of Crashes"
  ) +
  scale_fill_manual(values = c("Fatal Only" = "#2D3748",
                               "Fatal & Injury" = "#4A5568",
                               "Injury Only" = "#718096")) +
  scale_x_continuous(labels = scales::comma, 
                     limits = c(0, max(crash_summary$n) * 1.15)) +
  theme_minimal() +
  theme(
    plot.title = element_text(face = "bold", size = 13),
...
accident_plot <- 
  ...
  aes(color = BOROUGH, alpha = .3) + geom_sf( size = 1) +
...
ggplot(combined)+ aes(HOUR, fill = SPEED.LIMIT > 40) +
  geom_density(position = "identity", alpha = 0.5, bw = 1) +
  ...
  labs(fill = "Speed limit of the road")
...
borough_keywords <- c("Manhattan", "Brooklyn", "Queens", "Bronx", "Staten Island")
...
twitter_nyc <- twitter_nyc %>%
  mutate(
    ...
    is_vehicle = str_detect(tolower(tweet_text), 
                            "car|vehicle|crash|accident|collision|highway|road|traffic")
  )
...
<<echo=FALSE, message=FALSE, warning=FALSE>>=
# Transit vs Vehicle Content Distribution
tweet_type_summary <- data.frame(
  Type = c("Public Transit", "Vehicle Crashes", "Other"),
  Count = c(
    sum(twitter_nyc$is_transit & !twitter_nyc$is_vehicle),
    nrow(twitter_vehicle),
    nrow(twitter_nyc) - sum(twitter_nyc$is_transit & !twitter_nyc$is_vehicle) - nrow(twitter_vehicle)
  )
) %>%
  mutate(Percentage = round(100 * Count / sum(Count), 1))

ggplot(data = tweet_type_summary) +
  aes(x = reorder(Type, -Count), y = Count, fill = Type) +
  geom_bar(stat = "identity") +
  geom_text(aes(label = paste0(Count, "\n(", Percentage, "%)")), 
            vjust = 1.5, size = 3) +
  labs(
    title = "NYC Traffic Tweet Content Distribution",
    x = "Content Type",
    y = "Number of Tweets"
  ) +
  scale_fill_manual(values = c("Public Transit" = "grey",
                               "Vehicle Crashes" = "#718096", 
                               "Other" = "#CBD5E0")) +
  theme_minimal() +
  theme(legend.position = "none")
@
...
vehicle_words <- twitter_vehicle %>%
  ...
  # Clean up the words
  filter(
    !str_detect(word, "^\\d+$"), # Remove pure numbers (like "95", "278")
    !word %in% c("nyc", "new", "york", "updated", "incident", "cleared",
                 "directions", "construction", "highway", "exit", "amp")
    # Remove traffic-specific common words that don't add meaning
  )
...
ggplot(data = top_words) +
  ...
  coord_flip() +
  labs(
    title = "Most Common Words in Vehicle Incident Tweets",
    x = "Word",
    y = "Frequency"
  ) +
  theme_minimal()
...
<<echo=FALSE, message=FALSE, warning=FALSE>>=
nyc2018 <- nyc %>%
  filter(year(CRASH.DATE) == 2018, BOROUGH != "")
# nrow(nyc2018) # 149234

twitter_borough <- twitter_vehicle %>%
  filter(location != "NYC (unspecified)") %>%
  # Convert to uppercase to match NYC data (BRONX → Bronx)
  mutate(location = toupper(location)) %>%
  count(location, name = "twitter_reports")

nyc_borough <- nyc2018 %>%
  count(BOROUGH, name = "nyc_crashes")

borough_comparison <- full_join(
  twitter_borough, 
  nyc_borough, 
  by = c("location" = "BOROUGH")
) %>%
  mutate(
    # If a value is NA, replace it with 0
    twitter_reports = ifelse(is.na(twitter_reports), 0, twitter_reports),
    nyc_crashes = ifelse(is.na(nyc_crashes), 0, nyc_crashes),
    
    # percentages of each borough's count
    twitter_pct = round(100 * twitter_reports / sum(twitter_reports), 1),
    nyc_pct = round(100 * nyc_crashes / sum(nyc_crashes), 1),
    
    # gap between twitter % and actual crash % (positive = over-reported on Twitter)
    difference = twitter_pct - nyc_pct
  ) %>%
  arrange(desc(nyc_pct))

borough_comparison

# reshaping data for ggplot
borough_comparison_long <- borough_comparison %>%
  select(location, twitter_pct, nyc_pct) %>%
  pivot_longer(
    cols = c(twitter_pct, nyc_pct),
    names_to = "source",
    values_to = "percentage"
  ) %>%
  mutate(
    source = case_when(
      source == "twitter_pct" ~ "Twitter Reports (n=66)",
      source == "nyc_pct" ~ "NYC Crashes (n=149K+)"
    ),
    
    location = str_to_title(location)
  )

ggplot(data = borough_comparison_long) +
  aes(x = reorder(location, -percentage), y = percentage, fill = source) +
  ...
  labs(
    title = "Social Media Reporting vs. NYC Motor Collision Crash Data by Borough",
    subtitle = "(2018 Data)",
    x = "",
    y = "Percentage",
    fill = ""
  ) +
  scale_fill_manual(values = c("Twitter Reports (n=66)" = "steelblue", 
                               "NYC Crashes (n=149K+)" = "grey")) +
  theme_minimal() +
  theme(
  ...
  ylim(0, max(comparison_long$percentage) * 1.15)
@
