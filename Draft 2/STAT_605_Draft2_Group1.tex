...
\hldef{ex_pas_move} \hlkwb{<-} \hldef{(}\hlopt{!}\hlkwd{is.na}\hldef{(nyv}\hlopt{$}\hldef{VEHICLE_OCCUPANTS))}\hlopt{&}\hldef{(nyv}\hlopt{$}\hldef{VEHICLE_OCCUPANTS} \hlopt{%in%} \hlnum{1}\hlopt{:}\hlnum{60}\hldef{)}
...
\hldef{ex_veh_year} \hlkwb{<-} \hldef{(}\hlopt{!}\hlkwd{is.na}\hldef{(nyv}\hlopt{$}\hldef{VEHICLE_YEAR))}\hlopt{&}
               \hldef{(nyv}\hlopt{$}\hldef{VEHICLE_YEAR} \hlopt{%in%} \hlnum{1920}\hlopt{:}\hlnum{2025}\hldef{)} \hlopt{&}
...
{\ttfamily\noindent\itshape\color{messagecolor}{\#\# `stat\_bin()` using `bins = 30`. Pick better value `binwidth`.}}\end{kframe}
...
\hldef{nyc}\hlopt{$}\hldef{HIGHWAY} \hlkwb{<-} \hldef{nyc}\hlopt{$}\hldef{round_location} \hlopt{%in%} \hldef{highways}\hlopt{$}\hldef{location}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlcom{# Fatality rate by hour}
\hldef{crashes_per_hour} \hlkwb{<-} \hlkwd{table}\hldef{(nyc}\hlopt{$}\hldef{HOUR)}
\hldef{fatalities_per_hour} \hlkwb{<-} \hlkwd{tapply}\hldef{(nyc}\hlopt{$}\hldef{NUMBER.OF.PERSONS.KILLED, nyc}\hlopt{$}\hldef{HOUR, sum,} \hlkwc{na.rm} \hldef{=} \hlnum{TRUE}\hldef{)}

\hldef{hourly_summary} \hlkwb{<-} \hlkwd{data.frame}\hldef{(}
  \hlkwc{HOUR} \hldef{=} \hlkwd{as.numeric}\hldef{(}\hlkwd{names}\hldef{(crashes_per_hour)),}
  \hlkwc{total_crashes} \hldef{=} \hlkwd{as.numeric}\hldef{(crashes_per_hour),}
  \hlkwc{total_fatalities} \hldef{=} \hlkwd{as.numeric}\hldef{(fatalities_per_hour)}
\hldef{)}
\hldef{hourly_summary}
\end{alltt}
\begin{verbatim}
...
\begin{alltt}
\hlcom{# Normalization }
\hlcom{# how many people die per 1,000 crashes at each hour}
\hldef{hourly_summary}\hlopt{$}\hldef{fatality_rate} \hlkwb{<-} \hldef{(hourly_summary}\hlopt{$}\hldef{total_fatalities} \hlopt{/} \hldef{hourly_summary}\hlopt{$}\hldef{total_crashes)} \hlopt{*} \hlnum{1000}

\hlkwd{ggplot}\hldef{(}\hlkwc{data} \hldef{= hourly_summary)} \hlopt{+}
  \hlkwd{aes}\hldef{(}\hlkwc{x} \hldef{= HOUR,} \hlkwc{y} \hldef{= fatality_rate)} \hlopt{+}
  \hlkwd{geom_point}\hldef{(}\hlkwc{size} \hldef{=} \hlnum{3}\hldef{)} \hlopt{+}
  \hlkwd{geom_smooth}\hldef{(}\hlkwc{method} \hldef{=} \hlsng{"lm"}\hldef{)} \hlopt{+}
  \hlkwd{labs}\hldef{(}\hlkwc{title} \hldef{=} \hlsng{"Fatality Rate by Hour of Day"}\hldef{,}
       \hlkwc{subtitle} \hldef{=} \hlsng{"Are nighttime crashes deadlier?"}\hldef{,}
       \hlkwc{x} \hldef{=} \hlsng{"Hour of Day (0 = Midnight, 23 = 11 PM)"}\hldef{,}
       \hlkwc{y} \hldef{=} \hlsng{"Fatalities per 1,000 Crashes"}\hldef{)}
\end{alltt}
