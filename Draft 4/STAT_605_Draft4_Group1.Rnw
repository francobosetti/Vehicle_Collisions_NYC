...
mutate(CRASH.DATE=as.Date(CRASH.DATE, format = "%m/%d/%Y"),
       ...
       covid_period = case_when(
         ...
         CRASH.DATE >= as.Date("2020-03-01") & CRASH.DATE < as.Date("2022-01-01") ~ "During COVID",
         CRASH.DATE >= as.Date("2022-01-01") ~ "Post-COVID"
       ),
       covid_period = factor(covid_period, 
                            levels = c("Pre-COVID", "During COVID", "Post-COVID")),
       is_covid = CRASH.DATE >= as.Date("2020-03-01") & CRASH.DATE < as.Date("2022-01-01"),
...
nyv <- nyv %>%
  mutate(
    covid_period = case_when(
      ...
      CRASH_DATE >= as.Date("2020-03-01") & CRASH_DATE < as.Date("2022-01-01") ~ "During COVID",
