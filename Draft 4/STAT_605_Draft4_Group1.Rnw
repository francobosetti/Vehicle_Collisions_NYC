...
twitter_words <- twitter_vehicle %>%
  ...
  filter(!str_detect(word, "^\\d+$"),
         !word %in% c("nyc", "new", "york", "updated", "incident", "cleared",
                     "directions", "construction", "highway", "exit", "amp", "https", "t.co")) %>%
...
factor_words <- nyc %>%
  ...
  count(word, sort = TRUE) %>%
  mutate(percentage = n / sum(n) * 100, source = "Official Reports") %>%
  head(12)

bind_rows(twitter_words, factor_words) %>%
  ggplot(aes(x = percentage, y = reorder_within(word, percentage, source), fill = source)) +
  geom_col(show.legend = FALSE) +
  facet_wrap(~source, scales = "free") +
  scale_y_reordered() +
  scale_fill_manual(values = c("Twitter" = "lightblue", "Motor Vehicle Collision" = "lightgreen")) +
...
speed_summary %>%
  kable(col.names = c("Period", "Total Crashes", "Speed-Related", "Percent"),
        digits = 1, caption = "Speed-Related Crashes by COVID Period (Text-Mined)")
@
